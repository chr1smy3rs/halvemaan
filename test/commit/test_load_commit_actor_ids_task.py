...
class LoadCommitActorIdsTaskTestCase(unittest.TestCase):
    ...
    def test_no_record_in_database(self):
        ...
        result = luigi.build([commit.LoadCommitActorIdsTask(owner='Netflix', name='dispatch-docker')],
    ...
    def test_record_in_database(self):
        ...
        result = luigi.build([commit.LoadCommitActorIdsTask(owner='google', name='ko')],
                             local_scheduler=True, detailed_summary=True)
    ...
    def _validate_commits(self, case_setup: CaseSetup):
        ...
        actual_actor_id_count = 0
        commits = case_setup.mongo_collection.find({'object_type': base.ObjectType.COMMIT.name})
        for item in commits:
            expected_actor_count += item['total_authors']
            actual_actor_id_count += len(item['authors'])
        self.assertEqual(expected_actor_count, actual_actor_id_count)

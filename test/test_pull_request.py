...
class LoadCommitIdsTaskTestCase(unittest.TestCase):
    """ Tests the loading of commit ids into the pull request documents into the mongo database """
    ...
    def test_no_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadCommitIdsTask(owner='Netflix', name='dispatch-docker')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=4, successful_tasks=4))
        ...
        valid = True
        for pr in prs:
            valid = valid and pr['total_commits'] == len(pr['commit_ids'])
        self.assertTrue(valid)
    ...
    def test_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadPullRequestsTask(owner='Netflix', name='mantis-cli')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=3, successful_tasks=3))
...
class LoadEditsTaskTestCase(unittest.TestCase):
    ...
    def test_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadPullRequestsTask(owner='google', name='omaha')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=3, successful_tasks=3))

        # test for load after repo data is loaded
        result = luigi.build([pull_request.LoadEditsTask(owner='google', name='omaha')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=2, successful_tasks=1, complete_tasks=1))
        prs = case_setup.mongo_collection.find({'object_type': base.ObjectType.PULL_REQUEST.name})
        valid = True
...
class LoadReactionsTaskTestCase(unittest.TestCase):
    """ Tests the loading of reactions into the pull request documents into the mongo database """
    ...
    def test_no_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadReactionsTask(owner='google', name='caliban')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=4, successful_tasks=4))
        ...
        valid = True
        for pr in prs:
            valid = valid and pr['total_reactions'] == len(pr['reactions'])
        self.assertTrue(valid)
    ...
    def test_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadPullRequestsTask(owner='google', name='cloud-forensics-utils')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=3, successful_tasks=3))

        # test for load after repo data is loaded
        result = luigi.build([pull_request.LoadReactionsTask(owner='google', name='cloud-forensics-utils')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=2, successful_tasks=1, complete_tasks=1))
        prs = case_setup.mongo_collection.find({'object_type': base.ObjectType.PULL_REQUEST.name})
        valid = True

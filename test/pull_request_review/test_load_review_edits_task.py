...
class LoadReviewEditsTaskTestCase(unittest.TestCase):
    """ Tests the loading of edits into the pull request review documents into the mongo database """
    ...
    def test_no_record_in_database(self):
        ...
        result = luigi.build([pull_request_review.LoadReviewEditsTask(owner='Netflix', name='frigga')],
        ...
        self.assertTrue(LoadReviewEditsTaskTestCase._validate_edits(case_setup))

    def test_record_in_database(self):
        """ checks for insert when repository is in database """
        case_setup = CaseSetup()

        result = luigi.build([pull_request_review.LoadReviewsTask(owner='Netflix', name='pollyjs')],
                             local_scheduler=True, detailed_summary=True)
        self.assertTrue(CaseSetup.validate_result(result, total_tasks=5, successful_tasks=5))

        # test for load after repo data is loaded
        result = luigi.build([pull_request_review.LoadReviewEditsTask(owner='Netflix', name='pollyjs')],
                             local_scheduler=True, detailed_summary=True)

...
class LoadCommentIdsTaskTestCase(unittest.TestCase):
    """ Tests the loading of comment ids into the pull request documents into the mongo database """
    ...
    def test_no_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadCommentIdsTask(owner='google', name='omaha')],
    ...
    def test_record_in_database(self):
        ...
        result = luigi.build([pull_request.LoadPullRequestsTask(owner='Netflix', name='mantis')],
                             local_scheduler=True, detailed_summary=True)
    ...
    def _validate_pull_requests(self, case_setup: CaseSetup):
        ...
        valid = True
        for pr in prs:
            valid = valid and pr['total_comments'] == len(pr['comment_ids'])
